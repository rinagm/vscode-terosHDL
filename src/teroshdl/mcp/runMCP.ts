...
function getToolConfiguration(): ToolConfiguration {
    const config = vscode.workspace.getConfiguration('teroshdl-mcp-server');
    ...
    return {
        ...
        teroshdl: enabledTools.teroshdl ?? true
    };
}
...
function updateStatusBar(port: number) {
    ...
    if (serverEnabled) {
        statusBarItem.text = `$(server) TerosHDL MCP Server: ${port}`;
        statusBarItem.tooltip = `MCP Server running at localhost:${port} (Click to toggle)`;
        statusBarItem.backgroundColor = undefined;
    } else {
        statusBarItem.text = `$(server) TerosHDL MCP Server: Off`;
...
export async function activateMCPServer(context: vscode.ExtensionContext) {
    logger.info('Activating teroshdl-mcp-server extension');
    ...
    try {
        ...
        statusBarItem.command = 'teroshdl.teroshdl-mcp-server.toggleServer';
        ...
        const toggleServerCommand = vscode.commands.registerCommand(
            'teroshdl.teroshdl-mcp-server.toggleServer', 
            () => toggleServerState(context)
        );

        const showServerInfoCommand = vscode.commands.registerCommand(
            'teroshdl.teroshdl-mcp-server.showServerInfo', 
            () => {
                if (serverEnabled) {
        ...
        const configChangeListener = vscode.workspace.onDidChangeConfiguration(async (event) => {
            if (event.affectsConfiguration('teroshdl-mcp-server.enabledTools')) {
        ...
        logger.error(`Failed to start TerosHDL MCP Server: ${error instanceof Error ? error.message : 'Unknown error'}`);
        vscode.window.showErrorMessage(`Failed to start TerosHDL MCP Server: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}

export async function deactivateMCPServer(context: vscode.ExtensionContext) {
    if (statusBarItem) {
    ...
    try {
        ...
        await mcpServer.stop();
        logger.info('TerosHDL MCP Server stopped successfully');
    } catch (error) {
        logger.error(`Error stopping TerosHDL MCP Server: ${error instanceof Error ? error.message : String(error)}`);
