...
import { registerTerosHDLTools } from './tools/teroshdl-tools';
...
export class MCPServer {
    ...
    public setupTools(): void {
        ...
        if (this.fileListingCallback) {
            ...
            // Register TerosHDL tools if enabled
            if (this.toolConfig.teroshdl) {
                registerTerosHDLTools(this.server);
                logger.info('MCP TerosHDL tools registered successfully');
            } else {
                logger.info('MCP TerosHDL tools disabled by configuration');
            }
        } else {
    ...
    public async start(): Promise<void> {
        try {
            ...
            logger.error(`[MCPServer.start] Failed to start TerosHDL MCP Server: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
