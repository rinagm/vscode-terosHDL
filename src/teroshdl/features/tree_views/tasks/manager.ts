...
export class Tasks_manager extends BaseView {
    ...
    public async runQuartusTask(task: e_taskType) {
        ...
        if (taskItem) {
            return await this.run(taskItem);
        }
    }
    ...
    async run(taskItem: element.Task) : Promise<p_result | undefined> {
        ...
        try {
            ...
            const result = await new Promise<p_result>((resolve, reject) => {
    ...
    public async clean(forceClean: boolean = false) : Promise<p_result | undefined> {
        if (this.checkRunning()) {
            return;
        }
