...
export function registerToolUserChatParticipant(context: vscode.ExtensionContext) {
    const handler: vscode.ChatRequestHandler = async (request: vscode.ChatRequest, chatContext: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) => {
        ...
        const tools = vscode.lm.tools.filter(tool => tool.tags.includes('teroshdl'));
        ...
        const runWithTools = async (): Promise<void> => {
            ...
            if (toolCalls.length) {
                for( const toolCall of toolCalls) {
                    // If the model called any tools, then we do another round- render the prompt 
                    // with those tool calls(rendering the PromptElements will invoke the tools)
                    ...
                    if (toolResultMetadata?.length) {
                        // Cache tool results for later, so they can be incorporated into later 
                        // prompts without calling the tool again
    ...
    const toolUser = vscode.chat.createChatParticipant('chat-teroshdl.tools', handler);
