...
/**
 * Command argument building utilities for GHDL
 */

export type GhdlCommandType = 'analyze' | 'elaborate' | 'run' | 'synth';

export interface GhdlCommandArgs {
    baseArgs: string[];
    runArgs: string[];
}

/**
 * Helper function to build GHDL command arguments from configuration
 ...
 */
export function buildGhdlArgs(
    config: e_tools_ghdl, 
    commandType: GhdlCommandType, 
    ...
    // Add VHDL standard (map enum values to GHDL standard values)
    ...
    switch (commandType) {
        ...
        case 'elaborate':
            ...
            break;
        case 'run':
            // Simulation-specific options that go AFTER the entity name
 ...
 */
function addSimulationArgs(config: e_tools_ghdl, runArgs: string[]): void {
