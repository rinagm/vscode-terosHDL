...
export function runTaskYosys(
    ...
    switch (taskType) {
        case e_taskType.YOSYS_LOAD_FILES:
            return runYosysLoadFiles(projectDefinition, callback);

        case e_taskType.YOSYS_ANALYZE:
            return runYosysAnalyze(projectDefinition, callback);

        case e_taskType.YOSYS_ELABORATE:
            return runYosysElaborate(projectDefinition, callback);

        case e_taskType.YOSYS_SYNTHESIS:
            return runYosysSynthesis(projectDefinition, callback);

        case e_taskType.YOSYS_COMPILE_ALL:
            return runYosysCompileAll(projectDefinition, callback);

        case e_taskType.YOSYS_SHOW:
            return runYosysShow(projectDefinition, callback);

        case e_taskType.OPENFOLDER:
            return runYosysOpenProjectFolder(projectDefinition, callback);
